...
def attackerAgainstNoninvertibleEmbedder(s, l, store_path, alpha, sameSeed=False):
    ...
    c1 = invertEmbedding(S, u, np.ones(l), l, x, y, alpha, sameSeed)
    # inverse embedding with v
    c2 = invertEmbedding(S, v, np.zeros(l), l, x, y, alpha, sameSeed)
    ...
    fake_s = nonInvertibleEmbedder(fake_w, fake_c_img, store_path, role, alpha)
